...
set xrange ["20100101":]
unset xlabel
set ylabel 'изменение сложности майнинга, %'
set logscale y2
set ytics 5
unset xtics
unset y2tics
unset grid
set grid x y
set output 'графики/изменение-сложности-майнинга-биткоинов.png'

delta_v(x) = ( vD = ((x - old_v) / old_v) * 100, old_v = x, vD)
old_v = NaN

plot \
  'data/bitcoin-blocks.dat' using 2:(delta_v($6)) with lines title 'изменение сложности майнинга' linetype 3 axes x1y1, \
  'data/bitcoin-blocks.dat' using 2:6 with lines title "сложность майнинга" linetype 7 axes x1y2
