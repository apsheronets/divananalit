...
let income market_rate difficulty hashrate power power_rate =
  let course = course difficulty hashrate power power_rate in
  let course = market_rate -. course in
  (25. *. course) /.
  (time_for_revard difficulty hashrate)

let income_per_day market_rate difficulty hashrate power power_rate =
  (income market_rate difficulty hashrate power power_rate) *. 60. *. 60. *. 24.

let amortizated_income hardware_cost market_rate difficulty hashrate power power_rate =
  let course = amortizated_course hardware_cost difficulty hashrate power power_rate in
  let course = market_rate -. course in
  (25. *. course) /.
  (time_for_revard difficulty hashrate)

let amortizated_income_per_day hardware_cost makret_rate difficulty hashrate power power_rate =
  (amortizated_income hardware_cost makret_rate difficulty hashrate power power_rate) *. 60. *. 60. *. 24.
...
let asics_income () =
  ...
  let income_per_day x =
    ...
    income_per_day market_rate difficulty 50000000000. 0. 3. in

  let amortizated_income_per_day x =
    let network_hashrate = network_hashrate +. (50000000000. *. x) in
    let difficulty = difficulty_for_hashrate network_hashrate in
    amortizated_income_per_day 2499. market_rate difficulty 50000000000. 0. 3. in
...
let lite_and_bit_comparison () =
  ...
  let bitcoin_income_per_day hashrate power power_rate =
    income_per_day market_rate difficulty hashrate power power_rate in
  let litecoin_income_per_day hashrate power power_rate =
    income_per_day litecoin_market_rate litecoin_difficulty hashrate power power_rate in
