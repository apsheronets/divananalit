...
let course ?(reward=25.) hashrate difficulty power_cost =
  let time_in_seconds = difficulty *. (2. ** 32.) /. hashrate in
  ((time_in_seconds /. 3600.) *. power_cost) /. reward

let course power_cost = course 400000000. 7672999.92016 (power_cost /. 30.)
...
let to_usd x = x /. 30.
let energy_for_revard hashrate power = (time_for_revard difficulty hashrate) *. power
let energy_per_coin hashrate power = (energy_for_revard hashrate power) /. revard

let course hashrate power power_rate =
  (energy_per_coin hashrate power) *. (to_usd (power_rate /. 3600000.))

let proofit_per_second hashrate power power_rate =
  (25. *. course hashrate power power_rate) /.
...
let amortizated_course hardware_cost hashrate power power_rate =
  ((proofit_per_second hashrate power power_rate +. amortization_per_second hardware_cost) *. (time_for_revard difficulty hashrate)) /. 25.

let market_rate = 100.

let income hashrate power power_rate =
  let course = course hashrate power power_rate in
  let course = market_rate -. course in
  (25. *. course) /.
  (time_for_revard difficulty hashrate)

let income_per_day hashrate power power_rate =
  (income hashrate power power_rate) *. 60. *. 60. *. 24.

let amortizated_income hardware_cost hashrate power power_rate =
  let course = amortizated_course hardware_cost hashrate power power_rate in
  let course = market_rate -. course in
  (25. *. course) /.
  (time_for_revard difficulty hashrate)

let amortizated_income_per_day hardware_cost hashrate power power_rate =
  (amortizated_income hardware_cost hashrate power power_rate) *. 60. *. 60. *. 24.
...
let () =
  ...
  (* BitForce SC 50 Gh/s *)
  P.color g green;
  P.fx g (course 50000000000. 1.) 0. 5.;
  P.color g red;
  P.fx g (amortizated_course 2499. 50000000000. 1.) 0. 5.;
